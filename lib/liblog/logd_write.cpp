...
#define LIBLOG_LOG_TAG "LOG_WRITE"
...
static int __write_to_log_daemon(log_id_t log_id, struct iovec *vec, size_t nr)
{
    ...
    #define MAX_SIZE 100
    struct iovec newVec[ MAX_SIZE + header_length];
    ...
    if (log_id == LOG_ID_SECURITY) {
            ...
            if ((uid != AID_SYSTEM) && (uid != AID_ROOT) && (uid != AID_LOG)) {
                ...
                if ((gid != AID_SYSTEM) && (gid != AID_ROOT) && (gid != AID_LOG)) {
                    ...
                    if ((gid != AID_SYSTEM) && (gid != AID_ROOT) && (gid != AID_LOG)) {
                        ...
                        groups = (gid_t*)calloc(num_groups, sizeof(gid_t));
                        if (!groups) {
                            return -ENOMEM;
                        }
        ...
        const char *tag =(const char*)vec[0].iov_base;
    ...
    if (logd_fd > 0) {
        ...
        if (snapshot) {
            ...
            buffer.header.tag = htole32(LIBLOG_LOG_TAG)[0];
